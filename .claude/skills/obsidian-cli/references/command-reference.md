...
> **Note:** `property:set` always stores `value=` as a string. For true array-typed properties (e.g. `tags`),
...
> **Note:** `template:insert` inserts into whichever file is currently active in the Obsidian UI. To create a new file from a template, use `obsidian create path="..." template="..."` instead.
...
> **Note:** `plugins versions` only shows version numbers for community plugins.
...
> **Note:** History is distinct from Sync version history. History uses Obsidian's built-in File Recovery snapshots.
...
Takes a screenshot of the Obsidian window. **Path must be vault-relative.**
...
Executes arbitrary JavaScript in the Obsidian app context. Has access to full Obsidian API.
